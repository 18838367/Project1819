...
ax1.plot(us.look_back_time((redshifts[len(redshifts)-len(mdustden):])[ind]),np.log10(mdustden[ind]*pow(h0,2.0)),'r', label ='Shark all metals')

ind = np.where(mdustden_mol > 0)
ax1.plot(us.look_back_time((redshifts[len(redshifts)-len(mdustden_mol):])[ind]),np.log10(mdustden_mol[ind]*pow(h0,2.0)),'r', linestyle = 'dashed', label ='Shark metals in molecular gas')
plt.show()

#calculating chi2 (or something vaguely similar to it)

print(us.look_back_time(xobs))
print(us.look_back_time((redshifts[len(redshifts)-len(mdustden_mol):])[ind]))
